...
<html lang="it" dir="ltr">
    ...
    <body>
                    ...
                    <div class="container">
                        <div class="d-flex">
	                        ...
	                        <a class="header-brand" href="<?=base_url()?>admin/dashboard">
                                ...
                                <img src="<?=base_url()?>assets/images/favicon.png" class="header-brand-img mobile-logo" alt="IEG Gestionale" />
                            </a>
                            <div class="d-flex order-lg-2 ml-auto">
                                ...
                                <div class="dropdown profile-dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow animated p-0">
                                        ...
                                        <a class="dropdown-item border-bottom" href="<?=base_url().config_item('admin_url')?>/profile"> <i class="dropdown-icon mdi mdi-account-edit"></i> Modifica profilo</a>
                                        <a class="dropdown-item border-bottom" href="<?=base_url().config_item('admin_url')?>/settings"> <i class="dropdown-icon mdi mdi-settings"></i> Configurazione</a>
                                        <a class="dropdown-item border-bottom" href="<?=base_url().config_item('admin_url')?>/login/logout"> <i class="dropdown-icon mdi mdi-logout-variant"></i> Esci</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <nav class="horizontalMenu clearfix">
                                ...
                                <ul class="horizontalMenu-list">
	                                <li aria-haspopup="true" class="<?php if($this->router->class=='dashboard'):?>active<?php endif; ?>">
                                        <a href="<?=base_url()?>admin/dashboard" <?php if ($this->router->class=='dashboard') echo 'class="sub-icon active"';?>>
                                    ...
                                    <li aria-haspopup="true" class="<?php if($this->router->class=='users'):?>active<?php endif; ?>">
                                      <span class="horizontalMenu-click"><i class="horizontalMenu-arrow fa fa-angle-down"></i></span>
                                     <a href="<?=base_url()?>admin/users" <?php if ($this->router->class=='users') echo 'class="sub-icon active"';?>>
                                        ...
                                        Utenti <i class="horizontal-icon"></i>
                                      </a>
                                    </li>
                                    <li aria-haspopup="true" class="<?php if($this->router->class=='practices'):?>active<?php endif; ?>">
                                            <span class="horizontalMenu-click"><i class="horizontalMenu-arrow fa fa-angle-down"></i></span>
                                            <a href="#" <?php if ($this->router->class=='practices') echo 'class="sub-icon active"';?>>
                                             <span class="hor-shape1"></span> <span class="hor-shape2"></span> <span class="hor-arrow"></span>
                                            ...
                                            <ul class="sub-menu">
                                             <li><a href="<?=base_url()?>admin/practices"> Lista Pratiche</a></li>
                                            </ul>
                                    </li>
                                    <li aria-haspopup="true" class="<?php if($this->router->class=='printconfig'):?>active<?php endif; ?>">
                                            <span class="horizontalMenu-click"><i class="horizontalMenu-arrow fa fa-angle-down"></i></span>
                                            <a href="<?=base_url()?>admin/printconfig" <?php if ($this->router->class=='printconfig') echo 'class="sub-icon active"';?>>
                                             <span class="hor-shape1"></span> <span class="hor-shape2"></span> <span class="hor-arrow"></span>
        ...
        <script type="text/javascript">
            ...
            <?php if($this->router->fetch_class() == "users" && (($this->router->fetch_method() == 'insert' || $this->router->fetch_method() == 'edit'))): ?>
                function validateUserForm() {
                    // var inp = document.getElementById('user_image');
                    // if(inp.files.length === 0){
                    //     swal('Please select profile image.', '', 'error');
                    //     inp.focus();
            ...
            <?php if($this->router->fetch_class() == "practices" && (($this->router->fetch_method() == 'insert' || $this->router->fetch_method() == 'edit'))): ?>
                function addBeneficiary()
                {
                    ...
                    $.ajax({
                        url: "<?=base_url().'admin/'.$this->router->fetch_class()?>/addbeneficiary/"+pid,
                ...
                function addContractor()
                {
                    ...
                    $.ajax({
                        url: "<?=base_url().'admin/'.$this->router->fetch_class()?>/addcontractor/"+pid,
                        success: function(data) {
                ...
                $(".auto_contractor").autocomplete({
                  source: function (request, response) {
                    $.ajax({
                      url: "<?=base_url()?>admin/practices/getcontractors",
                      type: "POST",
                      dataType: "json",
                      data: { search: request.term, _token: $("input[name=_token]").val() },
                ...
                function AutoBeneficiary()
                {
                    $(".auto_beneficiary").autocomplete({
                      source: function (request, response) {
                        $.ajax({
                          url: "<?=base_url()?>admin/practices/getbeneficiaries",
                          type: "POST",
                          dataType: "json",
                          data: { search: request.term, _token: $("input[name=_token]").val() },
                ...
                // $("#p_contractor_name").autocomplete({
                //   source: function (request, response) {
                //     $.ajax({
                //       url: "<?=base_url()?>admin/practices/getcontractors",
                //       type: "POST",
                ...
                //         response($.map(data, function (item) {
                //           return {
                //             value: item.p_contractor_name,
                //             addr: item.p_contractor_address,
                //             vat: item.p_contractor_vat_no
                //           };
                //         }))
                //       }
                //     })
                ...
                //     //Note : the "value" object is created dynamically in the autocomplete' source Ajax' success function (see above)
                //     debugger;
                //     $("#p_contractor_name").val(ui.item.value);
                //     $("#p_contractor_address").val(ui.item.addr);
                //     $("#p_contractor_vat_no").val(ui.item.vat);
                //   },
                //   messages: {
                //     noResults: "",
                //     results: function (resultsCount) { }
                ...
                // $("#pb_beneficiary_name_1").autocomplete({
                //   source: function (request, response) {
                //     $.ajax({
                //       url: "<?=base_url()?>admin/practices/getbeneficiaries",
                //       type: "POST",
                ...
                //     //Note : the "value" object is created dynamically in the autocomplete' source Ajax' success function (see above)
                //     debugger;
                //     $("#pb_beneficiary_name_1").val(ui.item.value);
                //     $("#pb_beneficiary_address_1").val(ui.item.addr);
                //     $("#pb_beneficiary_vat_no_1").val(ui.item.vat);
                //   },
                //   messages: {
                //     noResults: "",
                //     results: function (resultsCount) { }
